...
def train_and_evaluate(cfg, datasets, output_path):
    ...
    for epoch in range(cfg.eval_params.epochs):
        ...
        if not cfg.eval_params.skip_training:
            ...
            cfg.wandb_params.tags = ["training", f"reward_{cfg.rl_params.reward_function}", "no_buffer", f"lr{str(cfg.rl_params.rl_lr).replace('.', '_')}"]
        ...
        if not cfg.eval_params.skip_eval and epoch % cfg.eval_params.eval_frequency == 0:
            ...
            cfg.wandb_params.tags = ["evaluation", f"reward_{cfg.rl_params.reward_function}", "no_buffer", f"lr{str(cfg.rl_params.rl_lr).replace('.', '_')}"]
