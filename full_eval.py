    ...
    if not cfg.eval_params.skip_training:
        for epoch in range(cfg.eval_params.epochs):
            ...
            if not cfg.eval_params.skip_training:
                ...
                cfg.wandb_params.tags = ["training", "agent_pruning", "per_gaussian_reward",f"reward_{cfg.rl_params.reward_function}", f"lr{str(cfg.rl_params.rl_lr).replace('.', '_')}"]
...
def add_eval_tags(cfg):
    ...
    cfg.wandb_params.tags = ["evaluation", "default_pruning", "per_gaussian_reward", f"lr{str(cfg.rl_params.rl_lr).replace('.', '_')}"]
